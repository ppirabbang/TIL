			...
			struct stats_record *stats_prev)
{
	...
	/* Assignment#2: Print other XDP actions stats  */
	{
		...
		for(__u32 key=0; key < XDP_ACTION_MAX; key++){
			...
			printf(fmt, action, rec->total.rx_bytes, bps, period);
		}
	}
}
...
static bool map_collect(int fd, __u32 map_type, __u32 key, struct record *rec)
{
	...
	rec->total.rx_packets = value.rx_packets; // value = datarec in here
	rec->total.rx_bytes = value.rx_bytes; //total = datarec in struct
	return true;
}
